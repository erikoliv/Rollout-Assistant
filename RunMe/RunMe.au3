		...
		If UBound($aSelect) - 1 > 0 Then
			...
			For $i = 1 To UBound($aSelect) - 1
				...
				Else
					$aExe = $aSelect[$i][3]
				EndIf
				_ExtractIconFromExe($sRepository & $aExe, @TempDir & '\' & $i & '.bmp')
...
Func instalar()
	...
	If $aGetItem[1] <> '' Then
		$aSelect = ''
		_DBStart()
		_SQLite_GetTable2d(-1, StringFormat("Select Repository from Softwares Where Name Like '%s' And Version Like '%s'", $aGetItem[1], $aGetItem[2]), $aSelect, $iRows, $iColumns)
		_DBStop()
		If UBound($aSelect) - 1 > 0 Then
			...
			GUISetState(@SW_SHOW)
			Sleep(2000)
			RunWait(StringFormat('%s\%s', $sRepository, $aSelect[1][0]))
...
Func instalarLista()
	...
	If IsArray($aReturn) Then
		...
		For $i = 1 To UBound($aReturn) - 1
			...
			RunWait($sRepository & '\' & $aSelect[1][2])
